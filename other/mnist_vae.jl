...
function metafmt(level, _module, group, id, file, line)
    ...
    suffix *= string(" ", Dates.now())
    return color, prefix, suffix
end

global_logger(ConsoleLogger(meta_formatter=metafmt, right_justify=displaysize(stderr)[2]))
...
evalcb = throttle(() -> @info("Loss function", (-L̄(X[:, rand(1:N, M)]))), 30)
...
cd(@__DIR__)
sample = vcat([hcat(img.([modelsample() for i = 1:10])...) for j=1:10]...)
